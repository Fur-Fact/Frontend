...
const AddCard = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await baseInstance.post(`pets`, formData, {
        headers: {
    ...
    <div className='w-full flex flex-col items-center relative'>
      <div className='flex flex-col justify-center items-center w-[357px] h-[320px] bg-[#DFDFDF] rounded-3xl m-2 z-10 filter'>
        ...
        <div className='font-bold'>
          버튼을 눌러 기르고 계신 동물을 추가해주세요
        </div>
      </div>
      <div className='flex flex-row blur-sm justify-between w-[350px] m-2 z-10'></div>
      <div className='w-full blur-sm font-bold text-left m-2 z-10'>
        검사 결과
      </div>
      <div className='h-[480px] blur-sm overflow-scroll z-10'></div>
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className='py-8'>
          ...
          <div className='flex flex-row justify-between'>
            <input
              className='w-[164px] mr-1 text-xs px-4 h-14 my-1 border-2 rounded-xl border-solid border-[#E5E4E3]'
              placeholder='나이'
              type='text'
              name='age'
              value={petData.age}
            ...
            />
            <input
              className='w-[164px] ml-1 text-xs px-4 h-14 my-1 border-2 rounded-xl border-solid border-[#E5E4E3]'
              placeholder='몸무게'
              type='text'
              name='weight'
